...
def load_data(file_path):
  ...
  def __metadatize_row(row, headers_key):
    ...
    for i, value in enumerate(row):
      ...
      elif headers_key[i] == config.step_field:
        metadatum['step_id'] = str(value)
    return metadatum
...
def segment_data(X, metadata, meta_id, fixed_length):
  '''
  Segment X into sequences based off of metadata and meta_id.
  Args:
  ...
  set_logger.debug("Fixing sequence length padding/cutting, with length \
                    of: " + str(fixed_length))
  for i in range(len(new_X)):
    new_X[i] = shaping_utils.fix_vector_length(new_X[i], fixed_length)
